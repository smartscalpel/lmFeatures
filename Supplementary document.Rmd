...
## MZ=646.2973
```{r get.mz.646.30,echo=FALSE}
  idx<-which(abs(p$mz-646.296916)<1e-3)
  cP<-which.max(p$intensity[idx])
  ionI<-idx[cP]
  maxMZ<-p$mz[ionI]
```

```{r get.subset.646.30,echo=FALSE}
  idx <- which(abs(p$mz - maxMZ) < (ppm * 1e-6 * maxMZ))
  p. <- p[idx, ]
```

```{r cluster.mz.646.30,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE,echo=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.646.30,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE,echo=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (FALSE){#all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.646.30,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE,echo=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r get.quality.646.30,echo=FALSE,eval=TRUE}
  qualityTab<-data.frame(ion=maxMZ,
                         medOutlier=NA,
                         numClass=NA,
                         numZero=NA,
