...
```{r fit.lin.huber,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l.huber<-rlm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l.huber$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l.huber)
  par(op)
}
...
```{r ion.plots.845,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.845,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.845,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.845,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.846,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.846,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.846,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.846,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.651,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.651,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.651,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.651,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.600,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.600,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.600,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.600,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.602,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.602,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.602,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.602,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
## MZ=174.051
```{r get.mz.174}
  idx<-which(abs(p$mz-174.051)<5e-3)
  cP<-which.max(p$intensity[idx])
  ionI<-idx[cP]
  maxMZ<-p$mz[ionI]
```
...
```{r ion.plots.174,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.174,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.174,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.174,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r get.quality.174}
  ...
  table(pred0)
```
```{r quality.tab.174,results='asis'}
  print_tableP(qualityTab[,c(1:5)])
  print_tableP(qualityTab[,c(6:11)])
...
```{r ion.plots.794,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.794,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.794,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.794,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```
```{r fit.lin2.794,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
p.<-p.[idL]
  l<-lm(mz ~ time, data = p.)
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
...
```{r filter2.lm.794,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.890,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.890,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.890,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.890,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.646,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.646,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.646,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.646,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.430,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.430,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.430,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.430,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.1261,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.1261,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.1261,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.1261,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.1110,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.1110,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.1110,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.1110,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
...
```{r ion.plots.222,fig.width=8.5,fig.height=4.5,results='asis',cache=FALSE}
print(getScanPlot(p[scan==p$scan[ionI]]))
```
...
```{r cluster.mz.222,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  dissim1 = dist(p.$mz)
  dendro1 <- hclust(d = dissim1, method = 'ward.D2')
  ct1 <- cutreeDynamic(
    dendro1,
...
```{r fit.lin.222,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
  l<-lm(mz ~ time, data = p[id.1, ])
if (all(!is.na(l$coefficients))) {
  op <- par(mfrow = c(2, 2), oma = c(0, 0, 2, 0))
  plot(l)
...
```{r filter.lm.222,fig.width=10.5,fig.height=10.5,results='asis',cache=FALSE}
if (all(!is.na(l$coefficients))) {
  range(l$residuals[abs(rstandard(l)) <= 2]) -> bnd
  pl <- predict(l, p.)
  rl <- p.$mz - pl
